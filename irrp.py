...
def playback():
    ...
    for arg in args.id:
        if arg not in records:
            print("Id {} not found".format(arg))
            continue

        code = records[arg]

        def split(arr, n, len_thres):
            if len(arr) <= n:
                return;
            if len(arr[n]) < len_thres:
                split(arr, n+1, len_thres)
            else:
                off_periods = arr[n][1::2]
                max_idx = code.index(max(off_periods))
                arr.insert(n+1, arr[n][max_idx+1:])
                arr[n] = arr[n][:max_idx+1]

                split(arr, n, len_thres)
                split(arr, n+1, len_thres)
